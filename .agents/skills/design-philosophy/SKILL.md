...
> - **Move:** lift the label to the leading type role, demote the help text to a smaller size on the ramp (`--vscode-bodyFontSize-xSmall`), and make "Reset" a quiet secondary action so the row reads top-down.
...
<a id="type-ramp"></a>
### The type ramp - rank and two weights, not free sizes

Text styles express **rank in the hierarchy**, not arbitrary sizes. The
registered size tokens are `--vscode-bodyFontSize` (13, base),
`--vscode-bodyFontSize-small` (12), and `--vscode-bodyFontSize-xSmall` (11);
larger heading sizes are deliberate, structural exceptions rather than ramp
steps. There are exactly **two weights** - regular (400) and semiBold (600),
written as literals (no weight tokens are registered). There is no medium (500).
"Strong" is not a bigger size; it is the *same size at semiBold*.
- **Decision rule:** choose the size by the text's **rank**, not by eyeballing a
  px value. A heading that looks weak is *"the wrong rank"* or *"missing the
  semiBold weight,"* not *"bump the font to 14."* A `500` anywhere is **off the
  ramp**.
...
| "make this 14px" | "this should sit on the **body size ramp** (`--vscode-bodyFontSize`, `-small`, `-xSmall`)" | 4 · One thing leads |
