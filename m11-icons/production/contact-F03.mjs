...
const ROSTER = [
	['firestore', 'firestore', 'Firebase flame (file/firestore.svg’s 3-layer stack reads as plain bars at 8.20)', '#8A6018', 'file/firestore.svg #E0A23C → darkened (tone law)'],
	['flow', 'flow-typed', 'bent flow arrow (file/flow.svg, redrawn to the R9a floors)', '#4E545B', 'no brand hue earned → neutral'],
	['flutter', 'flutter', 'Flutter blade + folded chevron (file/flutter.svg)', '#12507F', 'brand #02569B → deepened (tone law)'],
	['forgejo', 'forgejo', 'anvil (the forge)', '#4E545B', 'no brand hue earned (git red is taken by the core git folder) → neutral'],
	['form', 'forms', 'clipboard with a filled field and a short button', '#4E545B', 'no brand → neutral'],
	['frontcommerce', '.front-commerce', 'shopping cart', '#4E545B', 'no brand hue in the inventory → neutral'],
	['functions', 'functions', 'λ (lambda)', '#4E545B', 'no brand → neutral'],
	['gamemaker', 'gamemaker', 'gamepad (grips, d-pad counter, button counter)', '#4E545B', 'no brand hue in the inventory → neutral'],
	['gcp', 'gcp', 'hollow cloud', '#1F63A8', 'brand Google blue #1A73E8 → darkened (tone law)'],
	['gemini', '.gemini', 'four-point spark', '#4A4A9E', 'brand Gemini blue-violet → darkened (tone law)'],
	['gemini-ai', 'gemini-ai', 'four-point spark + small spark', '#4A4A9E', 'brand Gemini blue-violet → darkened (R3 family with gemini)'],
	['generator', 'generated', 'regeneration arrow (open ring + head)', '#4E545B', 'no brand → neutral'],
	['gh-workflows', 'workflows', 'Actions ring + play head', '#2F6FB0', 'brand Actions blue #4E93D6 → darkened (tone law); mark from file/github-actions-workflow.svg'],
	['gitea', 'gitea', 'tea mug (the Gitea cup)', '#4A7A1E', 'brand #609926 → darkened (tone law)'],
	['gitea-workflows', 'workflows', 'Actions ring + play head', '#4A7A1E', 'brand Gitea #609926 → darkened (R3 family with gh-workflows: same mark, own hue)'],
	['global', 'global', 'wireframe globe (ring + equator + meridian)', '#4E545B', 'no brand → neutral'],
	['godot', '.godot', 'Godot robot head', '#2E7EA8', 'brand #478CBF → darkened (tone law)'],
	['grok', '.grok', 'xAI parallel slashes (file/grok.svg, widened to the R9a floors)', '#2F3338', 'brand xAI near-black → lifted just enough to stay under the tan'],
	['grunt', 'grunt', 'mallet (task runner)', '#4E545B', 'brand amber collides with the tan plate → neutral'],
	['guard', 'guards', 'shield', '#4E545B', 'no brand → neutral'],
	['gulp', 'gulp-tasks', 'cup with a straw', '#8E3A3B', 'brand gulp red #CF4647 → darkened (tone law)'],
	['haxelib', '.haxelib', 'Haxe four-blade pinwheel (file/haxe.svg)', '#9A5312', 'brand Haxe orange #EA8220 → darkened (tone law)'],
	['helper', 'helpers', '? (help)', '#4E545B', 'no brand → neutral'],
	['histoire', '.histoire', 'Histoire petal + tail (file/histoire.svg)', '#7E3A5A', 'file/histoire.svg #C4608F → darkened (tone law)'],
	['home', 'landing', 'house with a door counter', '#4E545B', 'no brand → neutral'],
	['husky', '.husky', 'husky head', '#4E545B', 'no brand hue in the inventory → neutral'],
	['idea', '.idea', 'lightbulb', '#4E545B', 'no brand → neutral'],
	['import', 'imports', 'arrow into a bracket', '#4E545B', 'no brand → neutral'],
	['include', 'includes', 'partial: an L-block and the piece that completes it', '#4E545B', 'no brand → neutral'],
	['input', 'inputs', 'text field with a caret', '#4E545B', 'no brand → neutral'],
	['instructions', 'instructions', 'bulleted steps (file/instructions.svg’s pennants read as a bare S-bend at 8.20)', '#4E545B', 'no brand → neutral'],
	['intellij', 'idea', 'IJ', '#3A3F45', 'brand JetBrains near-black → lifted just enough to stay under the tan'],
	['interceptor', 'interceptors', 'funnel (requests intercepted)', '#4E545B', 'no brand → neutral'],
	['interface', 'interface', 'UML provided-interface lollipop', '#4E545B', 'no brand → neutral'],
	['interfaces', 'interfaces', 'two UML lollipops', '#4E545B', 'no brand → neutral (R3 family with interface: a pair of lollipops)'],
	['ios', 'ios', 'apple', '#4E545B', 'no brand hue in the inventory (Apple grey) → neutral'],
	['javascript', 'javascript', 'JS', '#7E6A0C', 'brand JS #E8D44D → darkened (tone law)'],
	['jinja', 'jinja2', '{ } template braces', '#4E545B', 'brand maroon collides with the core git folder → neutral'],
	['job', 'jobs', 'clock (scheduled work)', '#4E545B', 'no brand → neutral'],
	['junie', '.junie', 'J', '#4E545B', 'no published brand hue that survives the tone law → neutral'],
	['keys', 'secrets', 'key (file/key.svg mark, redrawn to the R9a floors)', '#4E545B', 'no brand → neutral (file/key.svg gold is lighter than the tan)'],
	['kiro', '.kiro', 'K', '#6B3F8E', 'brand violet → darkened (tone law)'],
	['kubernetes', 'kubernetes', 'heptagonal helm ring', '#2F4BA0', 'brand #326CE5 → darkened (tone law)'],
	['kusto', 'kusto', 'magnifier (KQL query)', '#4E545B', 'brand azure collides with the slice’s gcp/godot blues → neutral'],
	['lefthook', 'lefthook', 'left hook (an arrow that drops in and turns hard left)', '#4E545B', 'no brand → neutral']
];
