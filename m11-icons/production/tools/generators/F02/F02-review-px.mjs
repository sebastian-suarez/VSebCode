...
const cells = ids.map(id => `<figure>
  <svg width="${SIZE}" height="${SIZE}" style="image-rendering:pixelated" viewBox="0 0 16 16">${inner(`${id}.svg`)}</svg>
  <svg width="${SIZE}" height="${SIZE}" style="image-rendering:pixelated" viewBox="0 0 16 16">${inner(`${id}-open.svg`)}</svg>
