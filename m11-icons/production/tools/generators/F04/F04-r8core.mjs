...
const CORE=['api','app','assets','components','config','coverage','db','dist','docker','docs','fonts','git','github','hooks','i18n','images','layout','library','log','middleware','mock','model','next','node','package','public','route','scripts','server','services','src','styles','temp','template','test','theme','types','utils','view','vscode'];
const jobs=[...EMBLEMS.map(e=>({id:e.id,base:uri('folder.svg'),icon:uri(e.id+'.svg')})),...CORE.map(id=>({id:'core:'+id,base:uri('folder.svg'),icon:uri(id+'.svg')}))];
...
for(let i=0;i<ids.length;i++)for(let j=i+1;j<ids.length;j++){
 ...
 const bothCore=ids[i].startsWith('core:')&&ids[j].startsWith('core:');if(!bothCore&&s>=0.60)hits.push([s,ids[i],ids[j]]);}
